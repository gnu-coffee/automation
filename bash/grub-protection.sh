...
case "$1" in
    -s|--set-password)
        ...
        if [[ "$CURRENT_STATUS" == "unset" ]]; then
            ...
            cp "$FILE_10_LINUX" "$FILE_10_LINUX.orig" && chmod -x $FILE_10_LINUX* && chmod +x "$FILE_10_LINUX" && if [ -f $FILE_10_LINUX.dpkg-dist ]; then chmod +x $FILE_10_LINUX.dpkg-dist; fi
...
EOF
    ...
    -u|--unset-password)
        ...
        if [[ -f "$FILE_40_CUSTOM.orig" && -f "$FILE_10_LINUX.orig" ]]; then
            ...
            cp "$FILE_10_LINUX.orig" "$FILE_10_LINUX" && chmod -x $FILE_10_LINUX* && chmod +x "$FILE_10_LINUX" && if [ -f $FILE_10_LINUX.dpkg-dist ]; then chmod +x $FILE_10_LINUX.dpkg-dist; fi
